...
The RC method is the most basic method. The pros and cons are easy to understand with background of engineering
mathematics and difficult to handle complex material with multiple poles.
...
\begin{equation}
  \begin{split}
    D(\omega) &= \epsilon_0\epsilon_r^*(\omega)E(\omega)\\
    & = \epsilon_0\epsilon_rE(\omega) + \epsilon_0\chi_p(\omega)E(\omega)\\
    & = \epsilon_0\epsilon_rE(\omega) + J_p(\omega)
  \end{split}
\end{equation}
After solving $J_p$ individually, it can attend the updating loop, and then $E$ can be solved via 
\begin{equation}
  E^n = \frac{D^n - J_p^n}{\epsilon_0\epsilon_r}
\end{equation}
Starting with the most general form of $\chi_p(\omega)$, the phasor polarization displacement can be written as
\begin{equation}
...
here. In real wrold implementation, leapfrog was picked out.
...
Rearrange
\begin{equation}
  \omega^2J_p(\omega) - j\omega\gamma_pJ_p(\omega) = -\epsilon_0\omega_p^2 E(\omega)
\end{equation}
